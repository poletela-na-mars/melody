		...
		<RadixSlider.Root defaultValue={[defaultVal]} value={[value]} onValueChange={handleChange} max={max} min={min}
			...
			</RadixSlider.Track>
			{
				...
				<>
					...
					<RadixSlider.Thumb data-tooltip-id='thumb-tooltip'
						data-tooltip-content={`${padStartWithZero(currentTime.min)}:${padStartWithZero(currentTime.sec)}`}
						data-tooltip-place='top'
						className={`block opacity-0 hover:opacity-100 w-[8px] h-[8px] rounded-full bg-white 
						shadow-lg transition focus:outline-none`} aria-label='Slider Thumb' />
				</>
			}
			<RadixSlider.Thumb className={`block opacity-0 hover:opacity-100 w-[8px] h-[8px] rounded-full bg-white shadow-lg
